    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM tritond.exe");
#else
                QProcess::execute("pkill tritond");
#endif
            }
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to tritond
#ifdef Q_OS_WIN
    m_monerod = QApplication::applicationDirPath() + "/tritond.exe";
#elif defined(Q_OS_UNIX)
    m_monerod = QApplication::applicationDirPath() + "/tritond";
#endif
