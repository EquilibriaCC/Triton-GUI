...
This is the GUI for the [core Triton implementation](https://github.com/TritonNetwork/TritonProtocol). It is open source and completely free to use without restrictions, except for those specified in the license agreement below. There are no restrictions on anyone creating an alternative implementation of Triton that uses the protocol and network in a compatible manner.

## License

See [LICENSE](LICENSE).

## Compiling the Triton GUI Wallet from source

### On Linux:

(Tested on Ubuntu 17.10 x64, Ubuntu 18.04 x64 and Gentoo x64)

1. Install Triton dependencies
...
3. Clone repository
    ...
    `git clone https://github.com/TritonNetwork/triton-gui.git`
...
3. Install [triton](https://github.com/TritonNetwork/TritonProtocol) dependencies:
...
6. Grab an up-to-date copy of the triton-gui repository
...
The Triton GUI on Windows is 64 bits only; 32-bit Windows GUI builds are not officially supported anymore.

1. Install [MSYS2](https://www.msys2.org/), follow the instructions on that page on how to update system and packages to the latest versions

2. Open an 64-bit MSYS2 shell: Use the *MSYS2 MinGW 64-bit* shortcut, or use the `msys2_shell.cmd` batch file with a `-mingw64` parameter

3. Install MSYS2 packages for Triton dependencies; the needed 64-bit packages have `x86_64` in their names

    ```
    pacman -S mingw-w64-x86_64-toolchain make mingw-w64-x86_64-cmake mingw-w64-x86_64-boost mingw-w64-x86_64-openssl mingw-w64-x86_64-zeromq mingw-w64-x86_64-libsodium
    ```

    You find more details about those dependencies in the [Triton documentation](https://github.com/TritonNetwork/TritonProtocol). Note that that there is no more need to compile Boost from source; like everything else, you can install it now with a MSYS2 package.
